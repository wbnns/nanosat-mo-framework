...
public class VectorPointingSimulator implements AttitudeProvider
{
  ...
  private Attitude angleStep(AbsoluteDate date, Frame frame)
  {
    ...
    if (isX && Math.abs(FastMath.toDegrees(diff.getX())) <= this.margin + 0.01) {
      isX = false;
      isY = true;
      Logger.getLogger(VectorPointingSimulator.class.getName()).log(Level.INFO, "Y");
    }

    if (isY && Math.abs(FastMath.toDegrees(diff.getY())) <= this.margin + 0.01) {
      isY = false;
      isZ = true;
      Logger.getLogger(VectorPointingSimulator.class.getName()).log(Level.INFO, "Z");
    }

    if (isZ && Math.abs(FastMath.toDegrees(diff.getZ())) <= this.margin + 0.01) {
      isZ = false;
      Logger.getLogger(VectorPointingSimulator.class.getName()).log(Level.INFO, "END");
    }
  ...
  Rotation angleStepAxis(Rotation currentRot, double angle, AXIS axis)
  {
    ...
    switch (axis) {
      case X:
        return new Rotation(currentRot.applyTo(Vector3D.PLUS_I), angle,
            RotationConvention.VECTOR_OPERATOR).applyTo(currentRot);
      case Y:
        return new Rotation(currentRot.applyTo(Vector3D.PLUS_J), angle,
            RotationConvention.VECTOR_OPERATOR).applyTo(currentRot);
      case Z:
        return new Rotation(currentRot.applyTo(Vector3D.PLUS_K), angle,
            RotationConvention.VECTOR_OPERATOR).applyTo(currentRot);
    }
    return null;
  }
